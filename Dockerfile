...
RUN apt-get update && apt-get install -y unixodbc odbcinst unixodbc-dev php7.2-odbc libgss3 php7.2-dev php-pear \
  ...
  && printf "; priority=20\nextension=/usr/lib/php/20160303/pdo_sqlsrv.so" > /etc/php/7.2/mods-available/pdo_sqlsrv.ini && ln -s /etc/php/7.2/mods-available/pdo_sqlsrv.ini /etc/php/7.2/apache2/conf.d/20-pdo_sqlsrv.ini \
