...
RUN apt-get update && apt-get install -y unixodbc odbcinst unixodbc-dev php7.1-odbc libgss3 php7.1-dev php-pear \
  ...
  && ./install.sh install --accept-license && odbcinst -q -d -n "ODBC Driver 13 for SQL Server" && dpkg -i /usr/local/src/libssl1.0.0_1.0.1t-1+deb8u6_amd64.deb && rm -rf /usr/local/src/* \
