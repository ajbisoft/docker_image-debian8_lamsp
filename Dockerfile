...
RUN apt-get update && apt-get install -y unixodbc odbcinst php7.1-odbc libgss3 php7.1-dev php-pear libssl-dev devscripts debhelper dh-exec dh-autoreconf libreadline-dev libltdl-dev \
  ...
  && apt-get -y --purge remove php7.1-dev php-pear libssl-dev devscripts debhelper dh-exec dh-autoreconf libreadline-dev libltdl-dev && apt-get -y --purge autoremove && apt-get clean && rm -rf /var/lib/apt/lists/* \
