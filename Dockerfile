...
RUN apt-get update && apt-get install -y unixodbc odbcinst php7.1-odbc libgss3 unixodbc-dev php7.1-dev php-pear libssl-dev devscripts debhelper dh-exec dh-autoreconf libreadline-dev libltdl-dev \
  &&  cd /usr/local/src/ && dget -u -x http://http.debian.net/debian/pool/main/u/unixodbc/unixodbc_2.3.1-3.dsc && cd unixodbc-*/ && dpkg-buildpackage -uc -us -B && cp -v ./exe/odbc_config /usr/local/bin/ \
  ...
  && apt-get -y --purge remove unixodbc-dev php7.1-dev php-pear libssl-dev devscripts debhelper dh-exec dh-autoreconf libreadline-dev libltdl-dev && apt-get -y --purge autoremove && apt-get clean && rm -rf /var/lib/apt/lists/* \
